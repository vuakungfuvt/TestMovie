...
//        }
    ...
    func setTabbarItem(_ title: String, activeIcon: String, deactiveIcon: String) {
        self.tabBarItem.image = UIImage(named: deactiveIcon)
        self.tabBarItem.selectedImage = UIImage(named: activeIcon)
        self.tabBarItem.title = title
        self.title = title
        let appearance = UITabBarItem.appearance(whenContainedInInstancesOf: [UIViewController.self])
        appearance.setTitleTextAttributes([.foregroundColor: UIColor.white, .font: R.font.nunitoSansRegular(size: 12) as Any], for: .normal)
        appearance.setTitleTextAttributes([.foregroundColor: R.color.appTintColor() as Any,
                                           .font: R.color.appTintColor() as Any], for: .selected)
    }
}
...
extension UIViewController {
    ...
    func makeToast(text: String) {
        self.view.makeToast(text, duration: 2, position: .bottom)
    }
    
    func showToastFavorite(isFavorite: Bool) {
        let message = isFavorite ? "Added to Favorite movies" : "Removed to Favorite movies"
        makeToast(text: message)
    }
    
    func showErrorMessage(errorContent: String) {
        let viewModel = AlertCustomViewModel(content: errorContent)
        AlertCustomViewController.present(from: self, animated: false) { vc in
            vc.setupViewModel(viewModel: viewModel)
            vc.modalPresentationStyle = .overFullScreen
